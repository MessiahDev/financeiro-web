...
export default function EmployeeDetailPage() {
  ...
  if (!selected) return <p className="text-sm text-slate-400 dark:text-slate-500 p-6">Funcionario nao encontrado.</p>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <dl className="grid grid-cols-2 gap-4 text-sm">
            <div><dt className="text-slate-400 dark:text-slate-500">Nome</dt><dd className="font-medium">{e.fullName}</dd></div>
            <div><dt className="text-slate-400 dark:text-slate-500">CPF</dt><dd>{formatCPF(e.cpf)}</dd></div>
            <div><dt className="text-slate-400 dark:text-slate-500">E-mail</dt><dd>{e.email}</dd></div>
            <div><dt className="text-slate-400 dark:text-slate-500">Cargo</dt><dd>{e.position ? positionMap[e.position] ?? String(e.position) : '—'}</dd></div>
            <div><dt className="text-slate-400 dark:text-slate-500">Departamento</dt><dd>{e.departmentName}</dd></div>
            <div><dt className="text-slate-400 dark:text-slate-500">Contrato</dt><dd>{e.contractType}</dd></div>
            <div><dt className="text-slate-400 dark:text-slate-500">Status</dt><dd><Badge variant={s.variant} dot>{s.label}</Badge></dd></div>
            <div><dt className="text-slate-400 dark:text-slate-500">Admissão</dt><dd>{formatDate(e.hireDate)}</dd></div>
            {e.terminationDate && <div><dt className="text-slate-400 dark:text-slate-500">Desligamento</dt><dd>{formatDate(e.terminationDate)}</dd></div>}
          </dl>
        </Card>
        <Card>
          <CardHeader title="Salário Atual" />
          <CardDivider />
          <p className="font-display text-2xl font-bold mt-2 text-slate-900 dark:text-slate-100">{formatCurrency(e.salary)}</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{e.currency}</p>
        </Card>
      </div>
    </div>
  )
