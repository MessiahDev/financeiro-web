...
function NewPayableForm({ onSubmit, onCancel, isSaving }: { onSubmit: (d: AccountPayableFormData) => Promise<void>; onCancel: () => void; isSaving: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
      ...
      <CurrencyInput label="Valor" required value={totalAmount} onChange={v => setValue('totalAmount', v, { shouldValidate: true })} error={errors.totalAmount?.message} />
...
function PayForm({ item, onSubmit, onCancel, isSaving }: { item: AccountPayable; onSubmit: (d: PayAccountPayableFormData) => Promise<void>; onCancel: () => void; isSaving: boolean }) {
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm({ resolver: zodResolver(payAccountPayableSchema), defaultValues: { amount: item.remainingAmount } })
  const amount = watch('amount')
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
      <p className="text-sm text-slate-500">Valor original: <strong>{formatCurrency(item.totalAmount)}</strong></p>
      <p className="text-sm text-slate-500">Valor restante: <strong>{formatCurrency(item.remainingAmount)}</strong></p>
...
export default function AccountsPayablePage() {
  ...
  const { items, isLoading, isSaving, page, pageSize, totalCount, totalPages, setPage, fetch, create, pay, cancel } = useAccountsPayable()
    ...
    { key: 'actions',      header: '', render: r => (
      <div className="flex items-center justify-end gap-1">
        {(r.status === AccountPayableStatus.Pending || r.status === AccountPayableStatus.Overdue) &&
          <Button size="sm" variant="ghost" onClick={() => { setTarget(r); setPayOpen(true) }}>Pagar</Button>}
        {(r.status !== AccountPayableStatus.Cancelled && r.status !== AccountPayableStatus.Paid) &&
          <Button size="sm" variant="ghost" onClick={() => { setTarget(r); setCancelOpen(true) }} className="text-red-500 hover:bg-red-50">Cancelar</Button>}
      </div>
    )},
  ]
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Modal isOpen={newOpen} onClose={() => setNewOpen(false)} title="Nova conta a pagar" size="md">
        <NewPayableForm isSaving={isSaving} onCancel={() => setNewOpen(false)}
          onSubmit={async d => { try { await create(d); success('Conta cadastrada!'); setNewOpen(false); fetch({ search }) } catch { notifyError('Erro ao cadastrar.') } }} />
      </Modal>

      <Modal isOpen={payOpen} onClose={() => setPayOpen(false)} title={`Pagar — ${target?.supplierName}`} size="sm">
        {target && <PayForm item={target} isSaving={isSaving} onCancel={() => setPayOpen(false)}
          onSubmit={async d => { try { await pay(target.id, d); success('Pagamento registrado!'); setPayOpen(false); setTarget(null) } catch { notifyError('Erro ao pagar.') } }} />}
      </Modal>

      <ConfirmModal isOpen={cancelOpen} onClose={() => setCancelOpen(false)}
        onConfirm={async () => { try { await cancel(target!.id, 'Cancelado pelo usuário'); success('Conta cancelada.'); setCancelOpen(false); setTarget(null) } catch { notifyError('Erro ao cancelar.') } }}
        title="Cancelar conta" message={`Cancelar conta de "${target?.supplierName}"?`}
        confirmLabel="Cancelar conta" isLoading={isSaving} />
    </div>
  )
}