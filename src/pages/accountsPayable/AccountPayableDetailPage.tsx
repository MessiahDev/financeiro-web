...
export default function AccountPayableDetailPage() {
  ...
  const { selected, isLoading, fetchById } = useCrud<AccountPayable, unknown, unknown>(accountsPayableService as never)
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title={a.description} subtitle={`Fornecedor: ${a.supplierName}`} backTo={ROUTES.ACCOUNTS_PAYABLE} />
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader title="Dados da Conta" />
          <CardDivider />
          <dl className="grid grid-cols-2 gap-4 text-sm">
            <div><dt className="text-slate-400">Fornecedor</dt><dd className="font-medium">{a.supplierName}</dd></div>
