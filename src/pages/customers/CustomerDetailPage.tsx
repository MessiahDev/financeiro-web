...
export default function CustomerDetailPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title={c.name} subtitle={formatDocument(c.document)} backTo={ROUTES.CUSTOMERS} />
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <dl className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div><dt className="text-slate-400">Documento</dt><dd>{formatDocument(c.document)}</dd></div>
            ...
            <div><dt className="text-slate-400">Tipo</dt><dd>{c.personType === 'Individual' ? 'Pessoa Fisica' : 'Pessoa Juridica'}</dd></div>
            ...
            {c.creditLimit !== undefined && <div><dt className="text-slate-400">Limite de Credito</dt><dd className="font-semibold text-blue-600">{formatCurrency(c.creditLimit)}</dd></div>}
          </dl>
          {c.notes && <>
            <CardDivider />
            <div><p className="text-xs text-slate-400 mb-1">Observacoes</p><p className="text-sm text-slate-700">{c.notes}</p></div>
          </>}
        </Card>
        <Card>
          <CardHeader title="Limite de Credito" />
          <CardDivider />
          <p className="font-display text-2xl font-bold mt-2 text-blue-600">{c.creditLimit !== undefined ? formatCurrency(c.creditLimit) : 'Nao definido'}</p>
        </Card>
      </div>
    </div>
  )
