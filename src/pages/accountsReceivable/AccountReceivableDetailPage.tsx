...
export default function AccountReceivableDetailPage() {
  ...
  const { selected, isLoading, fetchById } =
    useCrud<AccountReceivable, unknown, unknown>(accountsReceivableService as never)
  ...
  // único lugar correto de cálculo
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title={a.description}
        subtitle={`Cliente: ${a.customerName}`}
        backTo={ROUTES.ACCOUNTS_RECEIVABLE}
      />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <dl className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <dd className="font-medium">{a.customerName}</dd>
            </div>

            <div>
              <dt className="text-slate-400">Status</dt>
              <dd><AccountReceivableStatusBadge status={a.status} /></dd>
            </div>
            ...
            <div>
              <dt className="text-slate-400">Recebimento</dt>
              <dd>{a.receiptDate ? formatDate(a.receiptDate) : '-'}</dd>
            </div>

            <div>
              <dt className="text-slate-400">Valor Total</dt>
              <dd className="font-semibold">{formatCurrency(a.totalAmount)}</dd>
            </div>

            <div>
              <dt className="text-slate-400">Valor Recebido</dt>
              <dd className="text-green-600 font-semibold">
                {formatCurrency(a.receivedAmount)}
              </dd>
            </div>
          </dl>
        </Card>
        ...
        <Card>
          ...
          <p className={`text-3xl font-bold mt-2 ${isPaid ? 'text-green-600' : 'text-amber-600'}`}>
